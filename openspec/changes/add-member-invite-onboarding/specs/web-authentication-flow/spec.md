...
- **WHEN** an invited `member` submits valid temporary credentials on `/entrar`
- **THEN** the app establishes the authenticated session
- **AND** the router navigates the user to the first incomplete member onboarding step instead of the main app
...
#### Scenario: Invited member opens the app before completing first-login setup
- **WHEN** an authenticated `member` with onboarding status `pending_profile` opens a protected app route
- **THEN** the app redirects the user to member first-login onboarding