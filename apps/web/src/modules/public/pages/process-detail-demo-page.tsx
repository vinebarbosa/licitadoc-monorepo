...
function getDocumentStatusConfig(status: DocumentStatus) {
  const config = {
    ...
    generating: {
      ...
      bgColor: "bg-blue-50 dark:bg-blue-950/55",
      badgeClassName:
        "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/80 dark:text-blue-200 dark:border-blue-900",
    },
    error: {
      label: "Erro na geração",
      icon: XCircle,
      color: "text-red-600 dark:text-red-300",
      bgColor: "bg-red-50 dark:bg-red-950/55",
      badgeClassName:
        "bg-red-50 text-red-700 border-red-200 dark:bg-red-950/80 dark:text-red-200 dark:border-red-900",
    },
    generated: {
      label: "Gerado",
      icon: CheckCircle2,
      color: "text-emerald-600 dark:text-emerald-300",
      bgColor: "bg-emerald-50 dark:bg-emerald-950/55",
      badgeClassName:
        "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/80 dark:text-emerald-200 dark:border-emerald-900",
    },
  };
  return config[status];
}

const demoDocumentIcons = {
  DFD: ClipboardList,
  ETP: FileSearch,
  TR: ScrollText,
  Minuta: Scale,
...
function DocumentCard({ document }: { document: Document }) {
  ...
  const DocumentIcon = demoDocumentIcons[document.acronym as keyof typeof demoDocumentIcons];
    ...
    <Card className="group relative overflow-hidden transition-all hover:border-primary/30 hover:shadow-md">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-3">
            <div
              className={`flex h-12 w-12 items-center justify-center rounded-xl ring-1 ring-black/5 dark:ring-white/5 ${config.bgColor}`}
            >
              <DocumentIcon className={`h-6 w-6 ${config.color}`} />
            </div>
            <div className="">
