...
export type ProcessItemComponent = {
  id: string;
  title: string;
  description: string;
  quantity: string;
...
function getDepartmentCode(department: ProcessDepartmentReference) {
...
export function getEmptyProcessFormValues(organizationId = ""): ProcessFormValues {
  return {
...
function normalizeItemForRequest(
  item: ProcessItem,
  index: number,
): ProcessMutationItemRequest | null {
...
function buildProcessRequestBase(values: ProcessFormValues) {
  return {
...
export function buildCreateRequest(
  ...
  return {
    ...buildProcessRequestBase(values),
    ...(isAdmin ? { organizationId: values.organizationId } : {}),
  };
}

export function buildUpdateRequest(values: ProcessFormValues): ProcessUpdateRequest {
  return buildProcessRequestBase(values);
}

function formatIssuedAtForInput(issuedAt: string) {
  return issuedAt.slice(0, 10);
}

function getFormItemFromDetailItem(item: ProcessDetailResponse["items"][number]): ProcessItem {
  return {
    id: item.id,
    kind: item.kind,
    code: item.code,
...
export function mapProcessDetailToFormValues(process: ProcessDetailResponse): ProcessFormValues {
  return {
    formaContratacao: process.procurementMethod ?? "licitacao",
    modalidade:
      process.procurementMethod === "licitacao"
        ? (process.biddingModality ?? "pregao")
        : (process.biddingModality ?? ""),
    processNumber: process.processNumber,
    externalId: process.externalId ?? "",
    issuedAt: formatIssuedAtForInput(process.issuedAt),
    ...
    items: process.items.map(getFormItemFromDetailItem),
  };
}
...
export function ProcessFormWizard({
  ...
  useEffect(() => {
    if (!forcedOrganizationId || values.organizationId === forcedOrganizationId) {
      return;
    }
  ...
  }, [forcedOrganizationId, values.organizationId]);

  useEffect(() => {
    if (!defaultOrganizationId || values.organizationId) {
      return;
    }
    ...
    <main className="flex-1 overflow-auto bg-background">
      <div className="mx-auto max-w-7xl px-4 py-6 md:py-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
              ...
              <div className="space-y-6">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Confira todas as informações antes de salvar o processo.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="rounded-lg bg-muted/30 p-4">
                        <div className="mb-3">
                          ...
                          <p className="font-medium">{selectedOrg?.name ?? "Não informada"}</p>
                        </div>
                        <div>
                                      ...
                                      {item.components.map((component, componentIndex) => (
                                        ...
                                        >
                                          ...
                                          <span className="font-medium text-foreground/80">
                                            {component.title || "Componente sem título"}
                                          </span>
                    ...
                    <Alert>
                      ...
                      <AlertDescription>
                        Após salvar o processo, você poderá seguir com a geração ou revisão dos
                        documentos oficiais.
                      </AlertDescription>
                    </Alert>
                  </CardContent>
          ...
          <SummaryPanel departments={departments} organizations={organizations} values={values} />
        </div>
      </div>
    </main>
  );
